...
from game.maps import GAME_MAP
...
from game.state import Entity, GameState, LLMCharacter, Location, Player, Trait, World
...
def game_instance():
    ...
    with patch("pygame.init"), patch("pygame.font.init"), patch(
        "pygame.font.Font"
...
def test_draw_chat_bubble(mock_draw_rect, mock_get_ticks):
    from game import constants
    from game.rendering import draw_chat_bubble
...
def test_integration_dashboard_snapshot_regression(game_instance):
    ...
    # Multi-step flow
    game_instance.player.move(
        1, 0, GAME_MAP
    )  # Corrected: added GAME_MAP and only one direction
    game_instance.timeline_log.append(
...
def test_integration_entity_edge_names(game_instance):
    ...
    from game.state import Entity  # Import Entity here to ensure it's available
