...
from src.devscape import engine, state_scroll, db_scroll
...
def test_status_command_output(runner, mock_state, mock_onboarding_config, mock_prompts_config):
    with patch.object(state_scroll, 'load_state', return_value=mock_state):
        with patch.object(db_scroll, 'fetch_lineage', return_value=[]):
            with patch.object(engine.config_scroll, 'get_onboarding_config', return_value=mock_onboarding_config):
                                with patch.object(engine.config_scroll, 'get_prompts_config', return_value=mock_prompts_config):
                                    result = runner.invoke(engine.cli, ["status", "--name", "TestUser"])
                
                                    assert result.exit_code == 0
                                    assert "Welcome, TestUser, to the DevScape Ritual Status!" in result.output
                                    ...
                                    assert "--- Lineage Archive (Recent Entries) 📜 ---" in result.output
                                    assert "No lineage entries found." in result.output
                                    assert "May your journey be guided by wisdom and your glyphs be true." in result.output

def test_evolve_command(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        result = runner.invoke(engine.cli, ["evolve", "wisdom", "5", "--contributor", "TestUser"])

        assert result.exit_code == 0
        assert "Attempting to evolve trait 'wisdom' to level 5..." in result.output
        assert "Trait 'wisdom' evolved to level 5 and lineage inscribed." in result.output
...
def test_feedback_command(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        result = runner.invoke(engine.cli, ["feedback", "joy", "--contributor", "TestUser"])
...
def test_quest_start_command(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        result = runner.invoke(engine.cli, ["quest", "start", "test_quest_id", "--contributor", "TestUser"])
...
def test_quest_complete_command(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        result = runner.invoke(engine.cli, ["quest", "complete", "test_quest_id", "--contributor", "TestUser"])
...
def test_trigger_event_command(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        payload = {"trait_id": "intelligence", "new_level": 10, "contributor_id": "TestUser"}
        result = runner.invoke(engine.cli, ["trigger-event", "trait_evolved", json.dumps(payload)])

        assert result.exit_code == 0
        assert f"Event 'trait_evolved' triggered successfully with payload: {payload}" in result.output
        mock_publish.assert_called_once_with("trait_evolved", payload)

def test_trigger_event_command_invalid_json(runner):
    with patch.object(engine.event_bus, 'publish') as mock_publish:
        result = runner.invoke(engine.cli, ["trigger-event", "trait_evolved", "invalid_json"])
